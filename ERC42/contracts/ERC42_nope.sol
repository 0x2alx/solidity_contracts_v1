    ...
    Ownable
{
    ...
    mapping(address => bool) public _presale_mints;
    mapping(address => bool) public _public_mints;
    mapping(address => bool) public _phase_two_mints;
    ...
    uint245 public constant MAX_NFTS_PHASE_ONE = 2442;
    uint245 public constant MAX_NFTS_PHASE_TWO = 4242;
    ...
    //FORTY TWO FARMING
    uint256 private constant FARMER_GRACE_DAYS = 2;
    ...
    function empty_bag(
        ...
        unchecked {
            _SB_NFTS[token_id] -= _withdraw_amount;
    ...
    function fill_bag(
        ...
        unchecked {
            ...
            _SB_NFTS[token_id] += _extra_amount_fortytwo_tokens;
        }
    ...
    function mintSupremeBeingPhaseTwo(
        uint256 quantity
    ) external payable nonReentrant {
        require(
            block.timestamp > _last_phase_one_mint_tstamp + (86400 * 42 * 3),
            "Phase two not open yet!"
        );
        ...
        require(
            next_bag_id + quantity <= MAX_NFTS_PHASE_ONE + MAX_NFTS_PHASE_TWO,
            "Phase one mint over!"
        );
        require(
    ...
    function mintSupremeBeing(
        ...
        if (valid_merkle && next_bag_id <= 1420) {
    ...
    //TODO
    function burnSUPREMEBEING(uint256 id) external nonReentrant {
        require(
            _SB_NFTS[token_id].bag_owner == _msgSender() ||
                isApprovedForAll(_SB_NFTS[token_id].bag_owner, _msgSender()),
            "ERC1155: caller is not token owner nor approved"
        ...
        if (_SB_NFTS[token_id].bag_balance > 1 * (10 ^ 18)) {
            emit FORTYTWOTOKENRESERVEBURN(id, _SB_NFTS[token_id].bag_balance);
    ...
    function setPaymentRecipient(address addy) external onlyCreator {
